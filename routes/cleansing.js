...
async function autoExpireCleansingEntries(supabase) {
...
router.get('/list/:userId', auth, disableRLS, async (req, res) => {
  try {
    ...
    const supabase = req.supabaseAdmin || req.supabase;
    await autoExpireCleansingEntries(supabase);
...
router.put('/:cleansingId/update', auth, disableRLS, async (req, res) => {
  try {
    ...
    if (cleansing.expires_at && new Date(cleansing.expires_at) <= new Date() && !isTerminalStatus(cleansing.status)) {
      await supabase
        .from('cleansing_entries')
        .update({
          status: 'expired',
      ...
      return res.status(400).json({ error: 'Cleansing entry has expired' });
    }

    if (cleansing.status === 'expired') {
      return res.status(400).json({ error: 'Cleansing entry has expired' });
    }
...
router.patch('/:cleansingId/expiry', auth, disableRLS, async (req, res) => {
  try {
    const { cleansingId } = req.params;
    ...
    res.json({
      success: true,
      message: 'Cleansing expiry date updated successfully',
...
router.patch('/:cleansingId/expiry-status', auth, disableRLS, async (req, res) => {
  try {
    const { cleansingId } = req.params;
    ...
    res.json({
      success: true,
      message: active ? 'Cleansing entry reactivated successfully' : 'Cleansing entry marked as expired',
      data: updatedCleansing
    });
  } catch (error) {
    res.status(500).json({ error: 'Failed to update cleansing status', details: error.message });
  }
});
...
router.post('/:cleansingId/nodes/:nodeOrder/delay-notify', auth, disableRLS, async (req, res) => {
  try {
    ...
    const targetNode = cleansing.cleansing_workflow_nodes.find(n => n.node_order === targetNodeOrder);

    if (!targetNode) {
      return res.status(404).json({ error: 'Node not found' });
    }

    const canNotify = requester.role === 'admin' || cleansing.created_by === userId || targetNode.executor_id === userId;
    if (!canNotify) {
    ...
    cleansing.cleansing_assignments
      .filter(a => a.node_order === targetNodeOrder)
      .forEach(a => recipientIds.add(a.user_id));

    const finalMessage = message || `Delay reported on node ${targetNode.node_name} for cleansing entry ${cleansing.id}`;
...
async function sendConsolidatedCleansingEmail(recipients, cleansing, action, comment = '') {
  try {
    ...
    let subject = '';
    let message = '';
    const viewUrl = 'https://server.matrixtwin.com/cleansing';
