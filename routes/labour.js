...
async function autoExpireLabourEntries(supabase) {
...
router.get('/list/:userId', auth, disableRLS, async (req, res) => {
  try {
    ...
    const supabase = req.supabaseAdmin || req.supabase;
    await autoExpireLabourEntries(supabase);
...
router.put('/:labourId/update', auth, disableRLS, async (req, res) => {
  try {
    ...
    if (labour.expires_at && new Date(labour.expires_at) <= new Date() && !isTerminalStatus(labour.status)) {
      await supabase
        .from('labour_entries')
        .update({
          status: 'expired',
      ...
      return res.status(400).json({ error: 'Labour entry has expired' });
    }

    if (labour.status === 'expired') {
      return res.status(400).json({ error: 'Labour entry has expired' });
    }
    ...
    if (action === 'approve' && currentNode) {
      ...
      if (revertHistory && revertHistory.form_data) {
        console.log(`Reverting labour ${labourId} to data from node ${firstEditableNode.node_order}`);
      ...
      if (revertHistory && revertHistory.form_data) {
        console.log(`Reverting labour ${labourId} to data from node ${prevEditableNode.node_order}`);
...
router.patch('/:labourId/expiry', auth, disableRLS, async (req, res) => {
  try {
    const { labourId } = req.params;
    ...
    res.json({
      success: true,
      message: 'Labour expiry date updated successfully',
      data: updatedLabour
    });
  } catch (error) {
...
router.patch('/:labourId/expiry-status', auth, disableRLS, async (req, res) => {
  try {
    const { labourId } = req.params;
    ...
    res.json({
      success: true,
      message: active ? 'Labour entry reactivated successfully' : 'Labour entry marked as expired',
      data: updatedLabour
    });
  } catch (error) {
    res.status(500).json({ error: 'Failed to update labour status', details: error.message });
  }
});
...
router.post('/:labourId/nodes/:nodeOrder/delay-notify', auth, disableRLS, async (req, res) => {
  try {
    ...
    const targetNode = labour.labour_workflow_nodes.find(n => n.node_order === targetNodeOrder);

    if (!targetNode) {
      return res.status(404).json({ error: 'Node not found' });
    }

    const canNotify = requester.role === 'admin' || labour.created_by === userId || targetNode.executor_id === userId;
    if (!canNotify) {
    ...
    labour.labour_assignments
      .filter(a => a.node_order === targetNodeOrder)
      .forEach(a => recipientIds.add(a.user_id));

    const finalMessage = message || `Delay reported on node ${targetNode.node_name} for labour entry ${labour.id}`;
...
/**
 * @route   POST /api/labour/:labourId/restore
 * @desc    Restore labour entry from history
 * @access  Private
 */
router.post('/:labourId/restore', auth, disableRLS, async (req, res) => {
  try {
    const { labourId } = req.params;
    ...
    if (!canUpdate) {
      return res.status(403).json({ error: 'No permission to restore this labour entry' });
    }
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        labour_type: historyEntry.form_data.laborCategory || labour.labour_type,
        trade_type: historyEntry.form_data.workPackage || labour.trade_type,
        number_of_workers: parseInt(historyEntry.form_data.numberOfWorkers) || labour.number_of_workers,
        hours_worked: parseFloat(historyEntry.form_data.hoursWorked) || labour.hours_worked,
        work_description: historyEntry.form_data.workArea || labour.work_description,
        notes: historyEntry.form_data.reportedDelays || labour.notes
      })
      .eq('id', labourId);
    ...
    await supabase
      ...
      .insert([{
        ...
        node_order: labour.current_node_index
      }]);

    res.json({
      success: true,
      message: 'Labour entry restored successfully',
    ...
    res.status(500).json({ 
      error: 'Failed to restore labour entry',
      details: error.message 
    });
  }
});
